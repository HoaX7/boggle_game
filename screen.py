...
class Screen:
    ...
    # Text to be displayed on the ending screen
    COMPUTER_END_TITLE = 'Computer found {} words!'
    # Display the computer's score
    COMPUTER_END_SCORE = 'Computer\'s total score is: {}'
    ...
    STALE_TEXT = 'The game has not started yet!'
    STALE_TITLE = 'Game not Started!'
    ...
    def __init__(self):
        ...
        self.__endgame = Endgame() # Create Endgame for the computer to find all words
    ...
    def set_dict(self, dict):
        ...
        self.__dict = dict
        self.__endgame.set_dict(dict)

    def set_board(self, board):
        # Should be in boggle.py
        self.__board = board

    def set_raw_board(self, board):
        self.board = board
        self.__endgame.set_board(board)
    ...
    def brute_force_button(self):
        self.force = Button(self.top,
                                  text='Find Words', font=self.SIZES['small'],
                                  command=self.brute_force, fg=self.BUTTONS_FG,
                                  bg=self.BUTTONS_BG, width=10)
        self.force.pack(side=LEFT, padx=self.PAD)
    ...
    def computer_found_words(self):
        # computer found words
        self.computer_bank_container = Listbox(self.pane,
                                      bg=self.BOX_BG,
                                      fg=self.BOX_FG,
                                      font=self.SIZES['small'])
        self.pane.add(self.computer_bank_container)
        self.computer_bank_container.insert(END, 'Computer Found:', '')
        self.scrollbar = ttk.Scrollbar(self.computer_bank_container, orient=VERTICAL)
        self.scrollbar.pack(side=RIGHT, fill=Y)
        self.computer_bank_container.config(yscrollcommand=self.scrollbar.set)
        self.scrollbar.config(command=self.computer_bank_container.yview)
    ...
    def end(self, type='TIMEUP'):
        ...
        self.computer_end_title = Label(self.end_frame,
                               text=self.COMPUTER_END_TITLE.format(len(self.__bank)),
                               bg=self.GAME_WINDOW_COLOR,
                               font=self.SIZES['large'])
    ...
    def check_word(self, undo=False):
        if (self.__curr_word in self.__dict) and (self.__curr_word not in self.__bank) and (len(self.__curr_word) >= 4):
            self.__score += self.__endgame.points(self.__curr_word)
